...
def gconnect():
    ...
    conn = psycopg2.connect(database="catelog", user="postgres", password="babaji9lm", host="localhost", port="5432")
    c = conn.cursor()
    c.execute("SELECT  * FROM users WHERE email=%s", (login_session['email'],))
    res = c.fetchall()
    if len(res)==0:
        c.execute("INSERT INTO users VALUES(%s,%s);", (login_session['username'], login_session['email'],))
        conn.commit()
...
def catalog(category):
    if 'username' in login_session or 'state' not in login_session:
        user = login_session['email']
    else:
        user = "NON"
    ...
    c.execute("SELECT * FROM categories")
    sqlresults = c.fetchall()
    c.execute("SELECT * FROM item WHERE category=%s;", (category,))
    items=c.fetchall()
...
def description(category,item):
    ...
    conn = psycopg2.connect(database="catelog", user="postgres", password="babaji9lm", host="localhost", port="5432")
    c = conn.cursor()
    c.execute("SELECT * FROM item where name=%s and category=%s;", (item, category,))
    sqlresults = c.fetchall()
    state = login_session['state']
...
def edit(item):
    if 'username' not in login_session or 'state' not in login_session:
        return redirect('/')

    conn = psycopg2.connect(database="catelog", user="postgres", password="babaji9lm", host="localhost", port="5432")
    c = conn.cursor()
    user = login_session['email']
    c.execute("SELECT * FROM item where name=%s and user_email=%s;", (item, user,))
    sqlresults = c.fetchall()
    if len(sqlresults) == 0:
        return redirect('/')
    c.execute("SELECT * from categories")
...
def editform():
    ...
    conn = psycopg2.connect(database="catelog", user="postgres", password="babaji9lm", host="localhost", port="5432")
    c = conn.cursor()
    c.execute("select * from item where id=%s", (id,))
    user = c.fetchone()
    user = user[4]
    if login_session['email'] != user:
        return redirect('/')

    c.execute("UPDATE item SET name=%s,description=%s,category=%s where id=%s and user_email=%s;", (name, description, category, id, user,))
    conn.commit()
    return redirect('/')
...
def delete(item):
    ...
    if len(sqlresults)==0:
        return redirect('/')
    c.execute("DELETE FROM item where name=%s;", (item,))
    conn.commit()
    return redirect('/')
...
def itemform():
    ...
    category=request.form['category']
    user=login_session['email']
    c.execute("INSERT INTO item(name,description,category,user_email) VALUES(%s,%s,%s,%s);", (name, description, category, user,))
    conn.commit()
    return redirect("/")
...
def catelog_json():
    ...
    for row in sqlresults:
        ...
        c.execute("SELECT * from item where category=%s;", (row[1],))
        res = c.fetchall()
